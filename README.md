...
- 数据在进行络传输、本地对象的持久化和加载到内存时，序列化协议采用protobuf。
...
#### 几个核心问题
#####  NameNode集群
- 基本思路：根据Raft协议选举出Leader，Client向Leader结点发送请求，当leader写入相关操作的日志，并且通过心跳传递给Follower结点，Follower结点写入日志后，返回成功标识，当一半以上的结点写入成功后，Leader写入到状态机中，返回成功给客户端，Follower收到下一次心跳的消息后，也应用到状态机中。
- 容错：如果是Follower宕机，此时如果仍然有一半以上的Follower结点，集群仍然可以正常运行；如果是Leader宕机，就需要重新选举新的Leader结点，选举成功后，继续提供服务。
##### 多副本写功能
- 我们这里的多副本写策略是：星型写。
...
### demo结果展示
链接：https://pan.baidu.com/s/1aWJcjW1O9ZCIcPnGMIdfkg 
